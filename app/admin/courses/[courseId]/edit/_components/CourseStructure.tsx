  ...
  return (
    ...
    if(activeType === "chapter" ) {
        ...
        if (oldIndex==-1 || newIndex == -1){
            toast.error("could not find chapters to reorder");
            return ;
         }
    ...
    if (activeType === "lesson" && overType === "lesson"){
        ...
        if(!chapterId || overChapterId !==chapterId){
            toast.error("could not find chapters for lesson reordering");
            return ;
        }
        ...
        if(oldLessonIndex === -1 || newLessonIndex === -1){
            toast.error("could not find lessons to reorder");
            return ;
        ...
        if (courseId) {
            ...
            const reorderdLessonPromise = () => reorderLessons(chapterId, LessonsToUpdate, courseId);
...
function toggleChapter(chapterId:string) {
    setItems(items.map((chapter) =>
        chapterId=== chapter.id ? { ...chapter, isOpen: !chapter.isOpen } : chapter
      )
    );
  }
                                    ...
                                    <Card>
                                        ...
                                        <div className="flex items-center justify-between p-3 border-b border-border">
                                            <div className="flex items-center gap-2">
                                                <Button {...listeners} variant="ghost" size="icon" className="cursor-grab opacity-60 active:cursor-grabbing hover:opacity-100">
